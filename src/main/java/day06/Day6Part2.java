...
public class Day6Part2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Checking how long it'll take to get back to this");
		int secondRedistributionCount = getSecondRedistributionCount();
		System.out.println("It took " + secondRedistributionCount + " steps");
	}

	private static int getSecondRedistributionCount() {
		Set<List<Integer>> secondTracker = new HashSet<>();
		secondTracker.add(secondaryStartingPoint);
		int secondRedistributionCount = redistribute(secondaryStartingPoint, secondTracker, 0);
		return secondRedistributionCount;
	}
	...
	private static int redistribute(List<Integer> memoryBanks, Set<List<Integer>> redistributionTracker, int redistributionCount) {
		...
		if (!redistributionTracker.add(redistributedMemoryBank)) {
			secondaryStartingPoint = redistributedMemoryBank;
			return redistributionCount;
		} else {
